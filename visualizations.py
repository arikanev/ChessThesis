...
sns.histplot(data=df_full, x='CPL', hue='Cheat', kde=True, element='step')
...
plt.figure(figsize=(12, 6))
scatter = plt.scatter(df_full['ELO'], df_full['CPL'], c=dt.predict_proba(df_full[['CPL', 'ELO']])[:, 1], cmap='coolwarm')
...
df_results['ELO_Range'] = pd.cut(df_results['ELO'], bins=[0, 1500, 2000, 2500, 3000, 3500], labels=['0-1500', '1501-2000', '2001-2500', '2501-3000', '3001-3500'])
accuracy_by_elo = df_results.groupby('ELO_Range').apply(lambda x: (x['Actual'] == x['Predicted']).mean())
...
# 4. ROC Curve
fpr, tpr, _ = roc_curve(df_results['Actual'], df_results['Predicted'])
roc_auc = auc(fpr, tpr)
