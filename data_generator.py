...
class ChessSimulator:
    ...
    def get_best_move(self, fen: str, elo: int) -> str:
        logging.debug(f"Getting best move for FEN: {fen[:20]}... at ELO: {elo}")
        max_retries = 3
    ...
    def get_cpl(self, fen: str, move: str, elo: int) -> float:
        logging.debug(f"Calculating CPL for move: {move} at ELO: {elo}")
        best_move = self.get_best_move(fen, 3190)
        ...
        print("CPL: ", abs(best_move_score - actual_move_score))
    ...
    def simulate_move(self, fen: str, elo: int, is_cheating: bool, num_trials: int) -> float:
        ...
        for _ in range(num_trials):
            if is_cheating:
                move = self.get_best_move(fen, 3190)
            else:
...
def generate_dataset(
    ...
    num_trials: int = 5,
    elo_range: Tuple[int, int, int] = (500, 2800, 100),
    positions_file: str = 'board_positions_old.txt',
...
def plot_feature_distributions(dataset_file: str = 'chess_dataset.csv') -> None:
    ...
    plt.figure(figsize=(12, 6))
    plt.hist(non_cheat_df['ELO'], bins=50, alpha=0.5, label='Non-Cheating', color='blue')
    plt.hist(cheat_df['ELO'], bins=50, alpha=0.5, label='Cheating', color='red')
    plt.xlabel('ELO Rating')
    plt.ylabel('Frequency')
    plt.title('ELO Distribution for Cheating vs Non-Cheating Players')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    generate_dataset(
        ...
        num_trials=10,
        elo_range=(500, 2800, 100), # ELO ratings from 500 to 2800 in steps of 100
